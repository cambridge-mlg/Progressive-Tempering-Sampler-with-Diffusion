...
class EGNN_dynamics(nn.Module):
    def __init__(
        ...
        edges_rows = edges[0]  # .to(self.device)
        edges_cols = edges[1]  # .to(self.device)
        self.register_buffer('edges_rows', edges_rows)
        self.register_buffer('edges_cols', edges_cols)
    ...
    def forward(self, t, xs):
        n_batch = xs.shape[0]
        edges = self._cast_edges2batch(
            [self.edges_rows, self.edges_cols], n_batch, self._n_particles
        )
        ...
        h = torch.repeat_interleave(t, self._n_particles, dim=0)
    ...
    def _cast_edges2batch(self, edges, n_batch, n_nodes):
        key = (n_batch, edges[0].device)  # Use device as part of the cache key
        if key not in self._edges_dict:
            rows, cols = edges
            rows_total, cols_total = [], []
            for i in range(n_batch):
                rows_total.append(rows + i * n_nodes)
                cols_total.append(cols + i * n_nodes)
            rows_total = torch.cat(rows_total).to(edges[0].device)
            cols_total = torch.cat(cols_total).to(edges[0].device)

            self._edges_dict[key] = [rows_total, cols_total]
        return self._edges_dict[key]


class EGNN_atom(nn.Module):
    def __init__(
        self,
        n_particles,
        n_dimension,
        ...
        n_layers=6,
        recurrent=False,
        attention=True,  # attention is set to be true
        condition_time=True,
        tanh=False,
        agg="sum",
        ...
        self.egnn = EGNN(
            ...
            in_edge_nf=2,  # Updated: Edge features now include distance and bond information
            hidden_nf=hidden_nf,
            act_fn=act_fn,
            n_layers=n_layers,
            recurrent=recurrent,
        ...
        self.register_buffer('edges_cols', edges_cols)
        self.condition_time = condition_time
        assert self.condition_time, "Conditregisterion time must be True"

        atom_type_labels = torch.tensor(atom_type_labels, device=self.device)
        self.register_buffer('atom_type_labels', atom_type_labels)
        num_atom_types = len(set(atom_type_labels))
        self.atom_type_embedding_layer = nn.Embedding(
            num_atom_types, atom_type_embedding_dim
        )
        ...
        adj = torch.zeros(
            (self._n_particles, self._n_particles),
            dtype=torch.float32,
            device=self.device,
        )
    ...
    def forward(self, t, xs):
        ...
        x = xs.reshape(n_batch * self._n_particles, self._n_dimension).clone()
        time_emb = torch.repeat_interleave(t, self._n_particles, dim=0)

        # create atom type embeddings
        ...
        # Compute node indices within the molecule
        ...
        # x_final = x_final - x
        x_final = x_final.view(n_batch, self._n_particles * self._n_dimension)
        x_final = remove_mean(x_final, self._n_particles, self._n_dimension)
        return x_final
...
class EGNN(nn.Module):
    ...
    def forward(self, h, x, edges, edge_attr=None, node_mask=None, edge_mask=None):
        ...
        if self.has_virtual:
            h = torch.cat(
                [self.embedding(h[:, 1:]), self.virtual_embedding(h[:, 0].long())],
                dim=-1,
            )
        else:
            h = self.embedding(h)
