...
def evaluate_aldp(
    ...
    if isinstance(transform.transform, bg.mixed.MixedTransform):
        ...
        transform.transform, bg.internal.CompleteInternalCoordinateTransform
    ):
        transform_mode = 'internal'
    else:
        raise NotImplementedError(
            'The evaluation is not implemented ' 'for this transform.'
        )
    ...
    for i in range(ndims):
        if (
            z_test[:, i].cpu().data.numpy().max() > 5
            or z_test[:, i].cpu().data.numpy().min() < -5
        ):
            ...
            print("In plots:")
            print(
                f"Data not within [-5, 5]. Quantiles of z_test[:, {i}]: {np.quantile(data, [0.01, 0.1, 0.9, 0.99])}"
            )
            print(f"NaN count: {nan_count}, proportion: {nan_proportion:.4f}")
            print()
        htest, _ = np.histogram(
            z_test[:, i].cpu().data.numpy(), nbins, range=hist_range, density=True
        )
    ...
    eps = 1e-10
    kld_unscaled = np.sum(
        hists_test * np.log((hists_test + eps) / (hists_gen + eps)), axis=0
    )
    ...
    kld_ = np.concatenate(
        [kld[: (3 * ncarts - 6)], np.zeros(6), kld[(3 * ncarts - 6) :]]
    )
    ...
    kld_phi = (
        np.sum(htest_phi * np.log((htest_phi + eps) / (hgen_phi + eps)))
        * 2
        * np.pi
        / nbins
    )
    ...
    kld_psi = (
        np.sum(htest_psi * np.log((htest_psi + eps) / (hgen_psi + eps)))
        * 2
        * np.pi
        / nbins
    )
    ...
    nbins_ram = 64
    eps_ram = 1e-10
    hist_ram_test = np.histogram2d(
        phi_d, psi_d, nbins_ram, range=[[-np.pi, np.pi], [-np.pi, np.pi]], density=True
    )[0]
    hist_ram_gen = np.histogram2d(
        phi, psi, nbins_ram, range=[[-np.pi, np.pi], [-np.pi, np.pi]], density=True
    )[0]
    kld_ram = (
        np.sum(
            hist_ram_test * np.log((hist_ram_test + eps_ram) / (hist_ram_gen + eps_ram))
        )
        * (2 * np.pi / nbins_ram) ** 2
    )
    ...
    if metric_dir is not None:
        ...
        for kld_label, kld_ in zip(kld_labels, kld):
            kld_append = np.concatenate(
                [np.array([iter + 1, np.median(kld_), np.mean(kld_)]), kld_]
            )
        ...
        np.savetxt(
            ...
            kld_hist,
            delimiter=',',
            header='it,kld_phi,kld_psi,kld_ram',
            comments='',
        )
    ...
    if plot_dir is not None:
        ...
        for i in range(len(label)):
            ...
            plt.savefig(
                os.path.join(plot_dir, 'marginals_%s_%07i.png' % (label[i], iter + 1))
            )
            plt.close()
